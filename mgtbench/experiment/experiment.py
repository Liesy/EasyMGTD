...
class ThresholdExperiment(BaseExperiment):
    ...
    def __init__(self, detector, **kargs) -> None:
        super().__init__()
        self.detector = (
            [detector] if isinstance(detector, MetricBasedDetector) else detector
        )
...
class PerturbExperiment(BaseExperiment):
    ...
    def __init__(self, detector, **kargs) -> None:
        super().__init__()
        self.detector = (
            [detector] if isinstance(detector, PerturbBasedDetector) else detector
        )
        if not self.detector:
            raise ValueError("You should pass a list of detector to an experiment")
    ...
    def launch(self, **config):
        ...
        for detector_predict in predict_list:
            if len(detector_predict["train_pred"]) == 2:
                ...
                test_metric = self.cal_metrics(*detector_predict["test_pred"])
                final_output.append(
                    DetectOutput(name="", train=train_metric, test=test_metric)
                )
        return final_output
...
class SupervisedExperiment(BaseExperiment):
    ...
    def __init__(self, detector, **kargs) -> None:
        super().__init__()
        self.detector = (
            [detector] if isinstance(detector, SupervisedDetector) else detector
        )
        if not self.detector:
            raise ValueError("You should pass a list of detector to an experiment")
...
class IncrementalExperiment(BaseExperiment):
    ...
    def __init__(self, detector, **kargs) -> None:
        super().__init__()
        self.detector = (
            [detector] if isinstance(detector, IncrementalDetector) else detector
        )
        if not self.detector:
            raise ValueError("You should pass a list of detector to an experiment")
    ...
    def return_output(self, detector, pair=None, intermedia=None):
        if not pair and not intermedia:
            raise ValueError(
                "At least one text or intermedia should be given for predictioon "
            )
        if not intermedia:
            inter_preds, inter_labels = self.data_prepare(
                detector.detect(pair[0], disable_tqdm=True), pair[1]
            )
        else:
