...
def load_pretrained(
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        model_name_or_path, use_fast=True, padding_side="left", **config_kwargs
    )
    if (
        tokenizer.pad_token_id is None or tokenizer.pad_token_id == 64000
    ):  # 64000 for baichuan model (older version)
    ...
    if quantization_bit is not None:
        if quantization_bit == 8:
            ...
            require_version(
                "transformers>=4.30.1", "To fix: pip install transformers>=4.30.1"
            )
            require_version(
                "accelerate>=0.20.3", "To fix: pip install accelerate>=0.20.3"
            )
            require_version(
                "peft>=0.4.0.dev0",
...
def load_pretrained_supervise(
    model_name_or_path, kargs, quantization_bit=None
) -> Tuple[PreTrainedModel, PreTrainedTokenizer]:
    r"""
    Loads pretrained model and tokenizer.
