...
def load_TruthfulQA(cache_dir, path=None):
    if path is None:
...
def load_SQuAD1(cache_dir, path=None):
    if path is None:
...
def load_NarrativeQA(cache_dir, path=None):
    if path is None:
...
def load(name, path=None):
    if path is None:
        if name in ["TruthfulQA", "SQuAD1", "NarrativeQA"]:
            path = os.path.join(DATASET_DIR_OTHERS, f"{name}_LLMs.csv")
        else:
