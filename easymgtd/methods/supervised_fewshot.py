...
def train_mlp(
    X_train, y_train, input_dim, num_classes, hidden_dim=512, lr=1e-3, epochs=50
):
...
class FewShotDetector(BaseDetector):
    def __init__(self, name, **kargs):
        ...
        if not self.model or not self.tokenizer:
            ...
            if not model_name_or_path:
                raise ValueError(
                    "You should pass the model_name_or_path or a model instance, but none is given"
                )
            quantitize_bit = kargs.get("load_in_k_bit", None)
            self.model, self.tokenizer = load_pretrained_supervise(
                model_name_or_path, kargs, quantitize_bit
            )
        if not isinstance(self.model, PreTrainedModel) or not isinstance(
            self.tokenizer, PreTrainedTokenizerBase
        ):
            raise ValueError(
                "Expect PreTrainedModel, PreTrainedTokenizer, got",
                type(self.model),
                type(self.tokenizer),
            )
    ...
    def construct_prototype(self, embeddings, labels):
        ...
        for label_list, embed_list in zip(labels, embeddings):
            ...
            class_embeds = torch.cat(
                embed_list, dim=0
            )  # Concatenate the embeddings across all batches for this label

            # Compute the mean for each class (mean across all embeddings for a particular class)
            class_means[label_list[0]] = class_embeds.mean(
                dim=0
            ).cpu()  # Assuming the label is the same for all samples

            # Compute the covariance matrix for the class embeddings
            class_covs[label_list[0]] = torch.cov(
                class_embeds.T
            ).cpu()  # Compute covariance for each class
    ...
    def update_shot(self, k):
        ...
        self.class_mean = self.construct_prototype(
            embs, torch.tensor(few_shot_data["label"])
        )
...
class BaselineDetector(FewShotDetector):
    ...
    def predict_with_cosine_similarity(self, test_embedding):
        test_embedding = (
            test_embedding.unsqueeze(0) if test_embedding.dim() == 1 else test_embedding
        )  # Ensure (1, hidden_dim)
        similarities = {
            label: cosine_similarity(
                test_embedding.cpu(), class_mean.unsqueeze(0), dim=1
            ).item()
    ...
    def finetune(self, data, config):
        ...
        if config.need_finetune:
            training_args = TrainingArguments(
                ...
                save_strategy=(
                    "epoch" if config.need_save else "no"
            ...
            train_encodings = self.tokenizer(
                stages[0]["text"], truncation=True, padding=True
            )
            ...
            trainer = Trainer(
                ...
                optimizers=(
                    AdamW(self.model.parameters(), lr=config.lr),
                    None,
            ...
            trainer.train()
        all_data = {
            "text": list(data["train"][0]["text"]) + list(data["train"][1]["text"]),
...
class GenerateDetector(FewShotDetector):
    ...
    def distribution_calibration(self, query, base_means, base_cov, k, alpha=0.21):
        ...
        means_tensor = torch.stack([base_means[label] for label in class_labels]).to(
            query.device
        )  # [num_classes, feature_dim]
        covs_tensor = torch.stack(
            [base_cov[label] for label in class_labels]
        )  # [num_classes, feature_dim, feature_dim]
        ...
        _, index = torch.topk(
            dist, k, largest=False
        )  # Indices of k closest class means
        ...
        selected_means = torch.stack(
            [base_means[label] for label in selected_labels]
        ).to(
            query.device
        )  # [k, feature_dim]
        selected_covs = torch.stack([base_cov[label] for label in selected_labels]).to(
            query.device
        )  # [k, feature_dim, feature_dim]

        # Compute the calibrated mean
        mean = torch.cat(
            [selected_means, query.unsqueeze(0)], dim=0
        )  # [k+1, feature_dim]
        calibrated_mean = mean.mean(dim=0)  # [feature_dim]
        # Compute the calibrated covariance
        calibrated_cov = selected_covs.mean(dim=0) + alpha * torch.eye(
            selected_covs.shape[-1], device=query.device
    ...
    def data_augment(self, support_data, support_label, n_ways, samples=750):
        ...
        for i in range(n_ways):
            ...
            class_label = support_label[i][
                0
            ]  # Use the first label (all should be the same)
            ...
            for i in range(self.kshot):
                ...
                mean, cov = self.distribution_calibration(
                    class_data[i], self.class_means, self.class_covs, k=2
                )
                # print(mean)
                # Sample new data from the calibrated distribution
                dist = torch.distributions.MultivariateNormal(
                    mean.to(dtype=torch.float32), cov.to(dtype=torch.float32)
                )
        ...
        X_aug = torch.cat(
            [
                torch.cat([torch.cat(d, dim=0) for d in support_data]),
                torch.cat(sampled_data),
            ],
            dim=0,
        )
        Y_aug = torch.cat(
            [
                torch.tensor([l for labels in support_label for l in labels]),
                torch.tensor(sampled_label),
            ],
            dim=0,
        )
        return X_aug, Y_aug
        ...
        try:
            ...
            with torch.no_grad():
                logits = self.classifier(
                    all_embeds.to(dtype=torch.float32).to(self.model.device)
                )
    ...
    def finetune(self, data, config):
        ...
        if config.classifier == "SVM":
            ...
            X_train = X_aug.cpu().numpy()
            y_train = Y_aug.cpu().numpy()
            self.classifier = LogisticRegression(max_iter=1000).fit(
                X=X_train, y=y_train
            )
            predicts = self.classifier.predict(X_train)
            acc = f1_score(y_train, predicts, average="macro")
            ...
            self.classifier = train_mlp(
                X_train, y_train, input_dim=X_train.shape[1], num_classes=num_classes
            )
            ...
            acc = f1_score(
                y_train.cpu().numpy(), predicts.cpu().numpy(), average="macro"
            )
            print(f"training Macro F1 Score: {acc:.4f}")
...
class RNDetector(FewShotDetector):
    ...
    def sample_episode(self, data):
        ...
        for label in sampled_classes:
            ...
            if len(texts) < self.kshot + self.num_query:
                raise ValueError(
                    f"Not enough samples for label {label} to perform k-shot and query sampling."
                )
    ...
    def finetune(self, data, config):
        ...
        support_texts, query_texts, support_labels, query_labels = self.sample_episode(
            all_data
        )
    ...
    def detect(self, text, disable_tqdm=True):
        ...
        for emb in new_embed:
            scores = {
                label: self.relation_net(
                    torch.cat([emb, prototype], dim=-1).to(self.model.device)
                ).item()
