...
class IG_block:
    ...
    def inverse(self, embeds, tar, max_features=20):
        ...
        max_ids = (
            torch.argsort(torch.abs(ig_attrs), dim=1)[:, -max_features:]
            .detach()
            .cpu()
            .numpy()
            .tolist()
...
class DemasqDetector(BaseDetector):
    ...
    def finetune(self, data, f_config):
        ...
        optimizer = optim.Adam(
            list(self.model.parameters()) + list(self.model.IG.model.parameters()),
            lr=0.0001,
        )
        ...
        for epoch in range(num_epochs):
            ...
            for inputs, targets in tqdm(
                ...
                e = enery(inputs_perm, targets) - min(
                    enery(inputs_perm, 1), enery(inputs_perm, 0)
                )
                e = e.cuda()
