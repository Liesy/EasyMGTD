...
class MetricBasedDetector(BaseDetector):
    def __init__(self, name, **kargs) -> None:
        ...
        if not self.model or not self.tokenizer:
            ...
            self.model, self.tokenizer = load_pretrained(
                model_name_or_path, quantitize_bit
            )
        ...
        for batch in tqdm(DataLoader(text)):
            with torch.no_grad():
                tokenized = self.tokenizer(
                    batch, max_length=1024, return_tensors="pt", truncation=True
...
class EntropyDetector(MetricBasedDetector):
    ...
    def detect(self, text, **kargs):
        ...
        for batch in tqdm(DataLoader(text)):
            with torch.no_grad():
                tokenized = self.tokenizer(
                    batch, truncation=True, max_length=512, return_tensors="pt"
                ).to(self.model.device)
                logits = self.model(**tokenized).logits[:, :-1]
...
class BinocularsDetector(BaseDetector):
    def __init__(self, name, **kargs) -> None:
        ...
        observer_name_or_path = kargs.get(
            "observer_model_name_or_path", "tiiuae/falcon-7b"
        )
        performer_name_or_path = kargs.get(
        ...
        self.observer_model = AutoModelForCausalLM.from_pretrained(
            observer_name_or_path, device_map=self.DEVICE_1, torch_dtype=torch.bfloat16
        )
        self.performer_model = AutoModelForCausalLM.from_pretrained(
            performer_name_or_path, device_map=self.DEVICE_2, torch_dtype=torch.bfloat16
        )
        ...
        self.BINOCULARS_ACCURACY_THRESHOLD = (
            0.9015310749276843  # optimized for f1-score
        )
        self.BINOCULARS_FPR_THRESHOLD = (
            0.8536432310785527  # optimized for low-fpr [chosen at 0.01%]
        )
    ...
    def perplexity(
        ...
        if median:
            ...
            ppl = (
                self.ce_loss_fn(shifted_logits.transpose(1, 2), shifted_labels)
    ...
    def entropy(
        ...
        ce = self.ce_loss_fn(input=q_scores, target=p_proba).view(
            -1, total_tokens_available
        )
    ...
    def find_threshold(self, train_scores, train_labels):
        ...
        if self.threshold_strategy == "default":
            assert (
                self.threshold == self.BINOCULARS_ACCURACY_THRESHOLD
                or self.threshold == self.BINOCULARS_FPR_THRESHOLD
            )
