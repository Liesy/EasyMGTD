...
class MetricTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        embeddings = embeddings[
            :, 0, :
        ]  # Take [CLS] token embedding (batch_size, hidden_dim)
    ...
    def forward(self, embeddings, labels):
        ...
        for i in range(sim_matrix.size(0)):  # Loop through batch
            ...
            positive_loss += torch.sum(
                alpha_p * torch.exp(-self.gamma * (S_p - self.O_p))
            )
            negative_loss += torch.sum(
                alpha_n * torch.exp(self.gamma * (S_n - self.O_n))
            )
            assert not torch.isnan(
                positive_loss
            ).any(), f"{S_p} found in exponential computation"
        ...
        assert not torch.isnan(
            positive_loss
        ).any(), "NaN found in exponential computation"
        assert not torch.isnan(
            negative_loss
        ).any(), "Inf found in exponential computation"
        return loss
        ...
        if num_labels == 2:
            ...
            for batch in tqdm(DataLoader(text), disable=disable_tqdm):
                with torch.no_grad():
                    ...
                    result.append(
                        self.model(**tokenized).logits.softmax(-1)[:, pos_bit].item()
                    )
        else:
            for batch in tqdm(DataLoader(text), disable=disable_tqdm):
                with torch.no_grad():
                    ...
                    result.append(
                        torch.argmax(self.model(**tokenized).logits, dim=-1).item()
                    )

        return result if isinstance(text, list) else result[0]

    def finetune(self, data, config):
        # Only flip labels for binary classification when pos_bit == 0
        num_labels = self.model.config.num_labels
        if num_labels == 2 and config.pos_bit == 0:
        ...
        experiment_name = (
            config.name if getattr(config, "name", "") else f"experiment_{self.name}"
        )
        swanlab_callback = SwanLabCallback(
            project=config.swanlab_project, experiment_name=experiment_name
        )
        ...
        training_args = TrainingArguments(
            ...
            logging_steps=config.logging_steps,  # Log steps
            weight_decay=config.weight_decay,  # Weight decay
            learning_rate=config.lr,  # Learning rate
            save_total_limit=config.save_total_limit,  # Limit to save only the best checkpoints
            gradient_accumulation_steps=config.gradient_accumulation_steps,
            report_to="none",  # Disable default integrations to avoid conflict
        )
