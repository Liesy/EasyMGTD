...
def perturb_texts_(args, texts, mask_model, mask_tokenizer, tokenizer, ceil_pct=False):
    ...
    if not args.random_fills:
        masked_texts = [
            tokenize_and_mask(x, span_length, buffer_size, pct, ceil_pct) for x in texts
        ]
...
class PerturbBasedDetector(BaseDetector):
    def __init__(self, name, **kargs) -> None:
        ...
        if not model_name_or_path or not mask_model_name_or_path:
            raise ValueError(
                "You should pass the model_name_or_path and mask_model_name_or_path, but ",
                model_name_or_path,
                mask_model_name_or_path,
                "are given",
            )
        # scoring/source model
        self.model, self.tokenizer = load_pretrained(model_name_or_path)
        # mask filling model
        self.mask_model, self.mask_tokenizer = load_pretrained_mask(
            mask_model_name_or_path
        )
...
class FastDetectGPTDetector:
    def __init__(self, name, **kargs) -> None:
        ...
        if not scoring_model_name_or_path:
            raise ValueError(
                "You should pass the scoring_model_name_or_path, but ",
    ...
    def get_sampling_discrepancy_analytic(self, logits_ref, logits_score, labels):
        ...
        log_likelihood = lprobs_score.gather(dim=-1, index=labels).squeeze(
            -1
        )  # get log likelihood of the correct token as in the original sequence (next token prediction)
...
class DNAGPTDetector(BaseDetector):
    ...
    def get_log_prob(self, text):
        tokenized = self.sampler.base_tokenizer(
        ...
        with torch.no_grad():
            ...
            return self.get_likelihood(
                logits_score, labels, self.sampler.base_tokenizer.pad_token_id
            )
